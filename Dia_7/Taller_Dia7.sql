...
-- 1- Escribe una consulta que permita agrupar los clientes de todos los empleados cuyo puesto sea responsable de ventas.
-- Se requiere que la consulta muestre: Nombre del cliente, teléfono, la ciudad, nombre y primer apellido del responsable
-- de ventas y su email.

CREATE INDEX puesto_empleado ON empleado(puesto);
...
-- 2- Se necesita obtener los registros de los pedidos entre el 15 de marzo del 2009 y el 15 de julio del 2009, para todos
-- los clientes que sean de la ciudad de Sotogrande. Se requiere mostrar el código del pedido, la fecha del pedido, fecha
-- de entrega, estado, los comentarios y el codigo del cliente que realizo dicho pedido.

CREATE INDEX registros_pedidos_Sotogrande ON pedido(fecha_pedido);
...
SHOW INDEXES FROM pedido;

-- 3- Se desea obtener los productos cuya gama pertenezca a las frutas y que el proveedor sea Frutas Talaveras S.A, 
-- se desea mostrar el código, nombre, descripción, cantidad en stock, y su precio con 10% de descuento, de igual forma
-- se pide la cantidad en los distintos pedidos que se han hecho.

CREATE INDEX productos_frutas ON producto(gama,proveedor);

SELECT p.codigo_producto,p.nombre,p.descripcion,p.cantidad_en_stock,p.precio_venta * 0.9 AS precio_con_descuento,dp.cantidad
FROM producto p 
...
CREATE table empleado(
	codigo_empleado int primary key not null auto_increment,
