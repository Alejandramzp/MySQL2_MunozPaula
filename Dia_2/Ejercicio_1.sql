...
DELIMITER //
CREATE PROCEDURE nombreD_presupuestoActual()
BEGIN
	SELECT nombre, presupuesto(presupuesto,gastos) AS valor_presupuesto -- FUNCION presupuesto PUNTO 11
	FROM departamento
	ORDER BY valor_presupuesto ASC;
END //
DELIMITER ;

CALL nombreD_presupuestoActual();
...
DELIMITER //
CREATE PROCEDURE empleados_departamento()
BEGIN 
	SELECT e.id,e.nombre,e.apellido1,e.apellido2,d.id AS id_departamento,d.nombre,d.presupuesto,d.gastos
	FROM empleado e
	INNER JOIN departamento d ON d.id = e.id_departamento;
END //
DELIMITER ;

CALL empleados_departamento();
...
DELIMITER //
CREATE PROCEDURE departamentoConEmpleados()
BEGIN 
...
-- presupuesto actual lo puede calcular restando al valor del presupuesto inicial (columna presupuesto) 
-- el valor de los gastos que ha generado (columna gastos).
DELIMITER //
CREATE PROCEDURE presupuesto_DepartamentoConEmpleados()
BEGIN 
	SELECT DISTINCT d.id, d.nombre, presupuesto(presupuesto,gastos) AS Presupuesto_Actual -- FUNCION presupuesto PUNTO 11
	FROM departamento d
	INNER JOIN empleado e ON e.id_departamento = d.id
	WHERE e.id_departamento IS NOT NULL;
END //
...
CALL presupuesto_DepartamentoConEmpleados();

-- 5.Devuelve el nombre del departamento donde trabaja el empleado que tiene el nif 38382980M.
DELIMITER //
CREATE PROCEDURE departamentoConNif()
BEGIN 
	SELECT d.nombre
	FROM departamento d
	INNER JOIN empleado e ON e.id_departamento = d.id
...
CALL departamentoConNif();

-- 6.Devuelve el nombre del departamento donde trabaja el empleado Pepe Ruiz Santana.
DELIMITER //
CREATE PROCEDURE departamentoPepe()
BEGIN 
	SELECT d.nombre
	FROM departamento d
	INNER JOIN empleado e ON e.id_departamento = d.id
	WHERE CONCAT(e.nombre,' ',e.apellido1,' ',e.apellido2) LIKE 'Pepe Ruiz Santana';
END //
DELIMITER ;

CALL departamentoPepe();
...
DELIMITER //
CREATE PROCEDURE departamentoID()
BEGIN 
	SELECT e.id,e.nif,e.nombre,e.apellido1,e.apellido2
	FROM departamento d
	INNER JOIN empleado e ON e.id_departamento = d.id
	WHERE d.nombre = 'I+D'
	ORDER BY e.nombre,e.apellido1,e.apellido2 ASC;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE departamento_Sis_Cont_ID()
BEGIN 
	SELECT e.id,e.nif,e.nombre,e.apellido1,e.apellido2,d.nombre
	FROM departamento d
	INNER JOIN empleado e ON e.id_departamento = d.id
...
DELIMITER //
CREATE PROCEDURE empleados_presupuesto()
BEGIN 
...
DELIMITER //
CREATE PROCEDURE departamento_apellido2Null()
BEGIN 
...
CALL departamento_apellido2Null();
